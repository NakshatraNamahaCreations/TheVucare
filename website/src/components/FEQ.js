...
function FEQ() {
  ...
  const postbanner = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://api.thevucare.com/api/userapp/addfeq",
        formData,
        {
          headers: {
  ...
  const getbannerimg = async () => {
    let res = await axios.get("http://api.thevucare.com/api/userapp/getallfeq");
    if (res.status === 200) {
  ...
  const deletebannerimg = async (id) => {
    axios({
      method: "post",
      url: "http://api.thevucare.com/api/userapp/deletefeq/" + id,
    })
                        ...
                        <tr key={i}>
                          ...
                          <td>
                            {element.img.map((image, j) => (
                              ...
                              src={`http://api.thevucare.com/feq/${image.data}`}
                                width={"100px"}
                                height={"50px"}
                                alt={`Image ${j + 1}`}
                                key={j}
