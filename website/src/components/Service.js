...
function Services() {
  ...
  const getservicemanagement = async () => {
    let res = await axios.get("http://api.thevucare.com/api/userapp/getservices");
    if ((res.status = 200)) {
  ...
  const deletecategory = async (id) => {
    axios({
      method: "post",
      url: "http://api.thevucare.com/api/userapp/deleteservices/" + id,
    })
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <img
          src={`http://api.thevucare.com/service/${row.serviceImg}`}
            width="50px"
            height="50px"
          />
        </div>
